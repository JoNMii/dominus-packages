Meteor.startup(function() {
  Cue.addJob('runBattle', {retryOnError:false, maxMs:1000*60*8}, function(task, done) {
    var battle = new BattleJob(task.data.gameId, task.data.x, task.data.y);
    battle.runBattle();
    done()
  });
});
  ...
  _.each(self.currentRound.armies, function(army) {
    ...
    switch(army.unitType) {
      case 'castle':
        ...
        Cue.addTask('updateNetCastle', {isAsync:true, unique:true}, {playerId: army.playerId});
      ...
      case 'village':
        ...
        Cue.addTask('updateNetVillages', {isAsync:true, unique:true}, {playerId: army.playerId});
      ...
      case 'army':
        ...
        if (army.destroyed) {
          ...
          Cue.addTask('updateNetArmies', {isAsync:true, unique:true}, {playerId: army.playerId});
        }
        break;
    }
...
BattleJob.prototype.trackLosesInPlayer = function(army) {
  ...
  if (num) {
    Cue.addTask('update_losses_worth',{isAsync:true, unique:true}, {playerId: army.playerId});
  }
}
...
BattleJob.prototype.gatherData = function() {
  ...
  var capitalFields = {playerId:1, gameId:1, x:1, y:1, user_id:1};
  ...
  if (capitalData) {
    self.currentRound.capital = capitalData;
  }
